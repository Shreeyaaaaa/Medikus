...
<div class="row">
       	 ...
       	 @foreach(App\Models\User::all() as $user)
                  @if($user->name=='Mark')
        <div class="offset-1 col-md-3 profile">
            <div class="card">
                <div class="card-body ">
                    <img  src="{{asset('images')}}/{{$user->image}}" class="profile-img" style="border-radius: 50%;" >
                    <br>
                </div>
            </div>
            </div>
                    <div class="col-md-8 profile ">
                   <h4 > Name:{{ucfirst($user->name)}}</h4>
                   <h4 > Degree:{{$user->education}}</h4>
                   <h4 > Specialist:{{$user->department}}</h4>
...
<div class=" offset-1 col-md-7">
   ...
   @foreach(App\Models\Appointment::all() as $b)
                             @if($b->user_id==$user->id)

            <form action="{{ route('booking.appointment') }}" method="POST">@csrf
            <div class="card">
                <div class="card-header lead">{{$b->date}}</div>
                <div class="card-body">
                    <div class="row">
                        ...
                        <input type="hidden" name="doctorId" value="{{$b->user_id}}">
               ...
               <div class="card-footer">
                @if(Auth::check())
                <button type="submit" class="btn btn-success" style="width: 100%;background-color:  #87ceeb">Book Appointment</button>
                @else
